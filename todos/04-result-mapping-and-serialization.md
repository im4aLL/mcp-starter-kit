...
- [ ] Domain tool content and `structuredContent` represent the same output.
- [ ] Valid prebuilt tool, resource, and prompt wire results are returned by identity and keep mixed content, roles, MIME ownership, and additional fields.
- [ ] Strings are not JSON quoted as resources or prompts.
- [ ] Unsupported resource domain values consistently throw `ResourceSerializationError` and no registration layer converts them to fake contents.
- [ ] No generic prose formatter, branded result helper, or second mapping pipeline is introduced.
