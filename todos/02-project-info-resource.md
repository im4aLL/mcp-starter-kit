...
- [ ] Add handler, decorator metadata, container resolution, registration, and mapper specs for string, object, array, finite number, boolean, `null`, `undefined`, circular object, `Date`, and `NaN` behavior.
- [ ] Add TSDoc above every introduced function, class, and class method.

## Acceptance criteria

- [ ] MCP discovery lists `project://info` and its decorator-owned `text/plain` hint.
...
- [ ] Domain JSON values serialize with `application/json` independently of the decorator metadata listing MIME hint.
...
- [ ] `ProjectInfoResource` is explicitly listed as a constructor and resolved from the same per-server container as `AddTool`.
...
- [ ] Confirm metadata listing does not instantiate `ProjectInfoResource` and runtime resolution does.
